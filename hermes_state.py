...
# ── fork: bit-packed messages.token_count accounting (hermes_token_codec) ──
# Module-level so `from hermes_state import _attach_token_view` keeps working for
# out-of-package consumers (agent/session_persistence.py JSONL snapshot writer).
# Upstream's mixin extraction moved the SessionDB methods that use these into
# hermes_state_messages.py / _usage.py; these helpers stay here (no import cycle:
# the mixins import them lazily inside the function body).
...
class SessionDB(
    ...
    _CONVERSATION_ROW_COLUMNS = (
        ...
        # fork: raw bit-packed token_count rides along so the replay projection can
        # attach a decoded `tokens` view (_rows_to_conversation -> _attach_token_view).
        "token_count"
    )
    ...
    def split_session(
        ...
        *,
        ...
        def _do(conn):
            ...
            #    COALESCE(?, col) is NOT the last-writer-wins billing regression
            #    (#48248): the child row was INSERTed one statement ago with NULL
            #    billing columns, so there is no earlier writer to overwrite. The
            #    first-writer-wins direction, COALESCE(billing_provider, ?), is
            #    update_token_counts' rule and lives in hermes_state_usage.py.
    ...
    def get_messages(self, session_id: str, *args, flatten_tokens: bool = True, **kwargs):
        """``SessionMessagesMixin.get_messages`` plus the fork's token flattening.

        The stored ``token_count`` is bit-packed and NEGATIVE for packed rows
        (hermes_token_codec). By default each returned row gets a readable
        ``tokens`` bucket dict and its scalar ``token_count`` reduced to a
        legacy-safe non-negative value, so dashboard / TUI / search / export
        consumers never read the raw packed sentinel.

        Pass ``flatten_tokens=False`` to keep the raw packed value untouched —
        ...
        accounting survives the round-trip (see gateway/platforms/api_server.py).

        A thin override rather than an edit inside upstream's mixin: the mixin's
        SQL/paging is upstream's, only the post-decode view is the fork's.
        """
        messages = super().get_messages(session_id, *args, **kwargs)
        if flatten_tokens:
            for msg in messages:
    ...
    def get_message_tokens(self, message_id: int) -> Dict[str, int]:
        ...
        row = self._read_one(
            "SELECT role, token_count FROM messages WHERE id = ?", (message_id,)
        )
        if row is None:
    ...
    def get_session_message_token_totals(self, session_id: str) -> Dict[str, int]:
        ...
        """
        row = self._read_one(
            _MESSAGE_TOKEN_TOTALS_SELECT + "WHERE session_id = ?", (session_id,)
        )
    ...
    def get_conversation_message_token_totals(
        ...
        return _row_to_token_totals(self._read_one(sql, (root_session_id,)))
    ...
    def get_message_token_timeseries(
        ...
        rows = self._read_all(sql, [bucket_seconds, bucket_seconds, *params])
