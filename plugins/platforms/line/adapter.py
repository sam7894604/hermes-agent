...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _maybe_notify_new_source(self, source: Dict[str, Any]) -> None:
        ...
        if not sid:
            return
        display = await self._resolve_chat_name(sid, stype) or sid
        await self._notify_admin_unauthorized(stype, sid, display=display)

    async def _reject_unauthorized(
        self, *, chat_id: str, chat_type: str, user_id: str,
        reply_token: str, attempt_text: str, dm: bool,
    ) -> None:
        """Unauthorized-source handling: notify admins (dedup), record the DM
        attempt, and send a throttled English reply."""
        display = await self._resolve_name(chat_id, chat_type, user_id) or user_id
        await self._notify_admin_unauthorized(chat_type, chat_id, display=display)

        if dm and self._whitelist is not None:
            try:
                self._whitelist.record_unauthorized_attempt(
                    chat_id,
                    {"user_id": user_id, "text": attempt_text, "ts": time.time()},
                )
            except Exception:
                logger.debug("LINE: record_unauthorized_attempt failed", exc_info=True)
